        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2.5">
            {quizzes.slice(0, 3).map((quiz) => {
              const isCompleted = quiz.hasPassed || quiz.attemptsCount > 0;
              return (
                <div
                  key={quiz.id}
                  className={`p-3.5 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all ${
                    isCompleted
                      ...
                      : 'bg-slate-100 dark:bg-slate-950/70 border-slate-200 dark:border-slate-800 hover:border-cyan-electric/30'
                  }`}
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isCompleted && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 shrink-0">
                          <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                          <span>تم الحل {quiz.bestScorePercentage !== null ? `(${quiz.bestScorePercentage}%)` : 'بنجاح'}</span>
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-chalk-muted">
                      ...
                      {quiz.attemptsCount > 0 && (
                        <>
                          <span>•</span>
                          <span className="text-emerald-500 font-bold">{quiz.attemptsCount} محاولات</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 self-end sm:self-auto shrink-0 ${
                      isCompleted
                        ? 'bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-chalk hover:bg-cyan-electric hover:text-black'
                  ...
                  >
                    ...
                    <span>{quiz.type === 'file' ? 'عرض الشيت' : isCompleted ? 'إعادة الاختبار' : 'بدء الاختبار'}</span>
                  </button>
                </div>
              );
            })}
