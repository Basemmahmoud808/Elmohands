...
export function StudentSubscriptionPaymentCard({
  ...
  return (
    <div className="space-y-6 font-arabic max-w-4xl mx-auto" dir="rtl">
      {/* Header Banner */}
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-slate-900 to-slate-950 border border-cyan-electric/30 text-chalk space-y-4 shadow-xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-72 h-72 bg-cyan-electric/10 rounded-full blur-3xl pointer-events-none -translate-x-1/2 -translate-y-1/2" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-electric/10 border border-cyan-electric/30 text-cyan-electric text-xs font-bold mb-1">
              ...
              <span>طرق الاشتراك الرسمية</span>
            </div>
            ...
            <p className="text-xs sm:text-sm text-chalk-muted font-medium max-w-2xl">
              اختر باقة الاشتراك المناسبة، وحوّل الرسوم بسهولة عبر فودافون كاش أو إنستاباي، ثم أرسل إشعار التحويل عبر واتساب ليتم التفعيل فوراً!
            </p>
          </div>
      ...
      <div className="space-y-3">
        <h3 className="text-base font-black text-slate-900 dark:text-chalk flex items-center gap-2">
            ...
            return (
              <div
                ...
                className={`p-6 rounded-3xl cursor-pointer transition-all relative border flex flex-col justify-between space-y-4 ${
                  isSelected
                    ? 'bg-cyan-500/10 dark:bg-slate-900 border-cyan-electric shadow-lg shadow-cyan-electric/15 ring-2 ring-cyan-electric/40'
                    : 'bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
                }`}
              >
                {plan.popular && (
                  <span className="absolute -top-3 left-6 px-3 py-0.5 rounded-full text-[10px] font-black bg-cyan-electric text-slate-950 shadow-md">
                    الأكثر اختياراً ⭐
                  </span>
                )}
                ...
                <div className="pt-2">
                  <div
                    className={`w-full py-2.5 rounded-xl text-xs font-black text-center transition-all ${
                      isSelected
                        ? 'bg-cyan-electric text-slate-950 shadow-cyan-glow'
                        : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-chalk-muted'
                    }`}
                  >
                    {isSelected ? 'تم اختيار هذه الخطة ✓' : 'اختيار الخطة'}
                  </div>
                </div>
              </div>
            );
      ...
      <div className="space-y-3">
        <h3 className="text-base font-black text-slate-900 dark:text-chalk flex items-center gap-2">
          ...
          <span>طريقة التحويل والمبلغ المطلوب:</span>
        </h3>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-6 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-rose-500/10 border border-rose-500/20 space-y-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300">
                حوّل مبلغ <strong className="text-slate-900 dark:text-chalk font-black">{selectedPlan.price} ج.م</strong> على محفظة فودافون كاش:
              </p>
              <div className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-slate-950 border border-rose-500/30">
                <span className="font-mono font-black text-base text-slate-900 dark:text-chalk" dir="ltr">
                  {paymentPhone}
                </span>
                <button
                  onClick={handleCopyNumber}
                  className="px-3 py-1 rounded-lg bg-rose-500 text-white text-xs font-bold hover:bg-rose-600 transition-colors flex items-center gap-1"
                >
            ...
            <div className="p-5 rounded-2xl bg-purple-500/10 border border-purple-500/20 space-y-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300">
                حوّل عبر تطبيق إنستاباي لرقم الهاتف أو عنوان المحفظة:
              </p>
              <div className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-slate-950 border border-purple-500/30">
                <span className="font-mono font-black text-base text-slate-900 dark:text-chalk" dir="ltr">
                  {paymentPhone}
                </span>
                <button
                  onClick={handleCopyNumber}
                  className="px-3 py-1 rounded-lg bg-purple-600 text-white text-xs font-bold hover:bg-purple-700 transition-colors flex items-center gap-1"
                >
                  {copiedNumber ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                  <span>{copiedNumber ? 'تم النسخ' : 'نسخ الرقم'}</span>
                </button>
          ...
          <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-700 dark:text-amber-300 leading-relaxed space-y-1">
            <p className="font-bold flex items-center gap-1.5">
              <span>⚠️ تنبيه هام بعد إتمام التحويل:</span>
            </p>
            <p>
              يرجى التقاط لقطة شاشة (Screenshot) واضحة تبيّن نجاح عملية التحويل ورقم العملية ورقم المحفظة التي قمت بالتحويل منها.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-base font-black text-slate-900 dark:text-chalk flex items-center gap-2">
          ...
          <span>إرسال إشعار التحويل لتفعيل الحساب:</span>
        </h3>

        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center space-y-4 shadow-sm">
          <div className="space-y-1">
            <h4 className="text-base font-black text-slate-900 dark:text-chalk">
              اضغط على الزر أدناه لإرسال الإيصال لمستر رضا خيرت مباشرة
            </h4>
