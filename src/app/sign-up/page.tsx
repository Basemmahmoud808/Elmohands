...
import { GraduationCap, Phone, User, BookOpen, ArrowLeft, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !phone.trim()) {
      setErrorMsg('يرجى ملء جميع الحقول بشكل صحيح');
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-chalk text-sm outline-none focus:border-cyan-electric transition-colors"
              >
                {GRADES.map((g, i) => (
